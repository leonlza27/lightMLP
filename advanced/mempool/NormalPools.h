...
public:
    ...
    bool isBusy() { return mallocLock.is_locked();}
};
...
private:
    uint16_t objnum;
    alignas(2) spin_lock mallocLock;
    char dataStart[32764];
public:
    mempoolMedium(){
        DataNode *lkhead = (DataNode*)dataStart;
        DataNode *lktail = (DataNode*)(dataStart + 32756);
        
        lkhead->end = 8;
        lkhead->startNext = 32756;
        lkhead->prev = -1;
        lkhead->thisStart = 0;
    ...
    void *pAlloc(uint16_t size){
        std::lock_guard<spin_lock> alloclk(mallocLock);
        size_t total_size = size + 8;
        size_t _needed = (total_size + 15) & (~15); 
    ...
    void pFree(void* ptr){
        ...
        if(ptr == 0) return;
        size_t delidx = ((size_t)ptr) - ((size_t)dataStart) - 8;
        if(delidx >= 32756 || delidx < 8) return;
